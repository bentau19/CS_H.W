...
int sepPlayersInit(char *players, player *targetArr, int playersNum) {
    ...
    if(i<playersNum||token!=NULL)return TRUE;
    return FALSE;
}
...
int isNotValid(int argc,char* argv[] ){
    ...
    if(argc!=4+NUM_OBJECTS+numPos)return (TRUE);
    int position2[numPos];
    memset(position2, 0, sizeof(position2));
    for(int i=0;i<numPos;i++){
        char *endVal;
        int current_pos = strtol(argv[i+4+NUM_OBJECTS],&endVal,10);
        if(current_pos==0)if(strcmp(argv[i+4+NUM_OBJECTS],"0")!=0)return TRUE;
        if(current_pos>=numPos||current_pos<0)return TRUE;
        if (position2[current_pos]==ERROR_SYNTAX)return TRUE;
...
{
    const char ERROR_MESSEGE[] = "INVALID INPUT";
    ...
    char players[NUM_PLAYERS*(PLAYER_LEN-1)];
...
void printLandingTableRow(int rowIdx,int rowDim){
...
void printGameSepLine(const int BOARD_DIM){
    ...
    for(int i=0;i<BOARD_DIM;i++)printf("%s",SEP_LINE);
    printf("\n");
}
...
void wordCenter(char* word){
    ...
    int currentHalfWay =(CELL_LEN - strlen(word))/2;
...
//this will do the action
        ...
        if(isFlipped(currentTable,coord1,coord2))
            return invalidMove(BOARD_DIM,currentTable,objects,positions);
        if(successPosition==ERROR_SYNTAX)
            return invalidMove(BOARD_DIM,currentTable,objects,positions);
...
/// set the first table
void setCurrentTable(const int BOARD_DIM, char currentTable[][MAX_SIZE_BOARD][MAX_SIZE_WORD]) {
    for (int i = 0; i < BOARD_DIM; i++) {
